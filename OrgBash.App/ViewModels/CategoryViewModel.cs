    ...
    public class CategoryViewModel : ViewModelBase, ICategoryViewModel
    {
       ...
       public void SaveState()
        ...
        private void InitializeCommands()
        {
            ...
            {
                IsBusy = true;
                if (await _bashClient.RateAsync(CurrentBashData.Id, AppConstants.TYPE_VALUE_POS, CurrentBashData.VotePosLinkParam))
                {
            ...
            {
                IsBusy = true;
                if (await _bashClient.RateAsync(CurrentBashData.Id, AppConstants.TYPE_VALUE_NEG, CurrentBashData.VoteNegLinkParam))
                {
                    _storedRatings.Value.Add(CurrentBashData.Id);
                    CurrentBashData.Rating--;
                    _bashClient.UpdateCache(BashCollection);
